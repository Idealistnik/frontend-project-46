...
const getDiffResult1 = (arr) => {
  ...
  const result = sorted.map(({
    ...
    switch (status) {
      ...
      case 'nested': {
        newLine = `    ${key}: ${getDiffResult1(value)}`;
        break;
      }
...
const firsrtFileInfo = getData('__fixtures__/file11.json');
const secondFileInfo = getData('__fixtures__/file22.json');
const firstFile = getParsedData(firsrtFileInfo, getFilePathExtension('__fixtures__/file11.json'));
const secondFile = getParsedData(secondFileInfo, getFilePathExtension('__fixtures__/file22.json'));
