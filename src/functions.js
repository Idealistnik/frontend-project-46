...
const getDiffList = (data1, data2) => {
  ...
  return keys.reduce((acc, key) => {
    if (!_.has(data1, key)) acc.push({ key, status: 'added', value: data2[key] });
    if (!_.has(data2, key)) acc.push({ key, status: 'deleted', value: data1[key] });
    if (_.isObject(data1[key]) && _.isObject(data2[key])) acc.push({ key, status: 'nested', value: getDiffList(data1[key], data2[key]) });
